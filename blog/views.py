...
def index(request):
    ...
    tech_articles = Article.objects.filter(article_type=Article.TECH_ART).order_by(
        "-updated"
    )[:6]
    context.update({"tech_articles": tech_articles})
    hobby_articles = Article.objects.filter(article_type=Article.HOBBY_ART).order_by(
        "-updated"
    )[:6]
...
def resume(request):
    ...
    experiences = JobExperience.objects.all().order_by("-start_date")
...
def me(request):
    ...
    article = Article.objects.filter(article_type=Article.MY_PROFILE).order_by(
        "-updated"
    )
    if article:
        article = article[0]
...
# Utils
def set_context(context=None, where_im_name: bool = None) -> dict:
    _context = {}
